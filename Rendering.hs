    ...
    where
        ...
        nextfs = foldr(\animI newfs ->
            case animI of
                ...
                (ParameterShift funcs steps) -> applyParameterShift funcs steps
                                                                    newfs frame
                (None)                       -> newfs)
            fs (fsAnimType fs)
    ...
    newfs = do
            ...
            (FS (fsFs fs)
                ...
                ( foldr (\(f,s) params -> f params (s*(fromIntegral frame)))
                    (fsParams fs) fsSteps )
                (fsAnimType fs))
            -- only thing that is different each run is frame count,
            -- fs always is the original fractal setting
...
psIterations (Param constRenderSettings constRootCols constInterpolates constIters
    ...
    where
        newParam = Param constRenderSettings constRootCols constInterpolates
            (floor $ currentDelta + (fromIntegral constIters)) constEpsilon
...
psEpsilon (Param constRenderSettings constRootCols constInterpolates constIters constEpsilon) currentDelta = newParam
    where
        newParam = Param constRenderSettings constRootCols
            constInterpolates constIters (currentDelta + constEpsilon)
...
psRootCols (Param constRenderSettings constRootCols constInterpolates constIters constEpsilon) currentDelta = newParam
    where
          newParam = Param constRenderSettings
                (newRootCols) constInterpolates constIters constEpsilon
          newRootCols = zipWith (\x y -> x y) test [0..(length constRootCols -1)]
          test = map (\x -> (newRootCol x)) constRootCols
                :: [Int -> (Complex Double,ColorW8)]
          newRootCol (rt,(r,g,b)) i = (rt,(clampRGB (redInterpolate $
                interpolXAtChannel i,greenInterpolate $
                interpolXAtChannel i,blueInterpolate  $
                interpolXAtChannel i))) :: (Complex Double,ColorW8)
                    -- interpolXAtChannel g,  interpolXAtChannel  b
          interpolXAtChannel i = ((round(currentDelta)) +
                ((i * (255 `div`(length constRootCols)))))`mod`256 :: Int
